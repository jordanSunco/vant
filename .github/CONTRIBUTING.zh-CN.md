...
以下是关于向 dvant 提交代码的指南。在向 dvant 提交 Issue 或者 PR 之前，请先花几分钟时间阅读以下文字。
...
- 所有的组件样式代码都位于 `packages/dvant-css/src` 下，`dvant-css` 也会在发布时单独发包
...
组件对应的样式需要放到 `packages/dvant-css/src` 目录下，`Button` 组件的话需要新建一个文件 `button.css`。如若个组件样式比较复杂，为了方便组织代码可以在 `packages/dvant-css/src` 下面新建一个同名目录 `button`，里面可以放一些 partial 样式。

新添加的 `button.css` 文件需要在 `packages/dvant-css/src/index.css` 中手动 import。
